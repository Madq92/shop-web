 ...
 */
export async function commonRequest<D>(url: string, params: ANY_OBJECT, method: RequestMethods, requestOption?: RequestOption, axiosOption?: AxiosRequestConfig): Promise<D> {
  const finalOption = {
  ...
  if (ajaxThrottleSet.has(url) && throttleFlag) {
    ...
    try {
      ...
      if (result.success) {
        ...
        showErrorMessage(result.errorMessage ? result.errorMessage : '服务器异常，请稍后重试');
        // 显示错误信息
        return Promise.reject(result);
      }
    } catch (error) {
 ...
 * @param axiosOption
 */
export const get = async <D>(url: string, params: ANY_OBJECT, options?: RequestOption, axiosOption?: AxiosRequestConfig) => {
 ...
 * @param axiosOption
 */
export const post = async <D>(url: string, params: ANY_OBJECT, options?: RequestOption, axiosOption?: AxiosRequestConfig) => {
 ...
 * @param axiosOption
 */
export const put = async <D>(url: string, params: ANY_OBJECT, options?: RequestOption, axiosOption?: AxiosRequestConfig) => {
 ...
 * @param axiosOption
 */
export const patch = async <D>(url: string, params: ANY_OBJECT, options?: RequestOption, axiosOption?: AxiosRequestConfig) => {
 ...
 * @param axiosOption
 */
export const del = async <D>(url: string, params: ANY_OBJECT, options?: RequestOption, axiosOption?: AxiosRequestConfig) => {
 ...
 * @param options 请求设置(showMask-是否显示Loading层，默认为true；showError-是否显示错误信息，默认为true；throttleFlag-是否开户节流，默认为false；throttleTimeout-节流时效，默认为50毫秒)
 */
export const download = async (url: string, params: ANY_OBJECT, filename: string, method: RequestMethods = 'post', options?: RequestOption) => {
 ...
 * @returns {Promise}
 */
export const downloadBlob = (url: string, params: ANY_OBJECT, method: RequestMethods = 'post', options?: RequestOption) => {
    ...
    commonRequest<Blob>(url, params, method, options, axiosOption)
      ...
      .catch(e => {
        if (e instanceof Blob) {
          ...
          reader.onload = () => {
            reject(reader.result ? JSON.parse(reader.result.toString()).errorMessage : '下载文件失败');
          };
          reader.readAsText(e);
        } else {
          reject('下载文件失败');
    ...
    commonRequest<ANY_OBJECT>(url, params, 'post', options, axiosOption)
      .then(res => {
        if (res?.success) {
          ...
          if (showError) showErrorMessage(res.data.errorMessage ? res.data.errorMessage : '数据请求失败');
          reject('数据请求失败');
        }
      })
      .catch(e => {
        if (showError) showErrorMessage(e.errorMessage ? e.errorMessage : '网络请求错误');
        reject(e);
      });
  });
};
